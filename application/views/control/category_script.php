<script>
	...
	function updateOrder_acces(dataID,FieldName,changeValue){
		var returnValue = $('#chkOrder'+dataID).val();
		console.log('returnValue:-'+returnValue)
		 if((changeValue=='')){
							swal({
			...
			$.post('<?php echo base_url('control/updateOrderCate_acces')?>',{ dataID:dataID , FieldName:FieldName , changeValue:changeValue }
				  ,function(data){ 
						if(data==1){
							 $.toast({
									heading: 'บันทึกข้อมูลสำเร็จแล้ว!',
				...
				$.ajax({
						 ...
						 success:function(data, status){
						  // console.log("File Uploaded");
							...
							if(oldImg==''){ window.location.href="<?php echo base_url('control/motocycle_category_add/')?>"+data;  } 
