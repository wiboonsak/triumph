				...
				<div class="row">
					...
					<div class="col-md-9 col-sm-8 col-xs-7">
						<div class="blog-main">
                                    ...
                                    <h4 class="pull-left" > Showing <span id="result"><?php echo $newsDetail->num_rows();?></span> results</h4>
                                </header>
							<div class="articles">
                                                            <?php
  $limit =''; $notUse ='';                            
$countAll=$this->Control_model->getNewsDetail1($limit,$notUse,'-100','-100');
...
foreach ($newsDetail->result() AS $data) {
    ...
    ?>
								<div class="article">
									...
									<p class="blog-text blog-text--article" ><?php echo $this->Control_model->str_limit_html($data->news_detail,300)?></p>
									<a href="<?php echo base_url('News/news_detail/').$data->id ?>" class="btn button button--main button--grey transparent">Read More</a>
									<span class="line line--title line--blog-title line--article"><span class="line__first"></span><span class="line__second"></span></span>	
								</div>
<?php  } ?>
							</div>
							...
							<div class="pagination wow slideInUp" data-wow-delay="0.7s" data-wow-duration="1.5s">
								<span class="pagination__text">Page <?php echo $page?> of <?php echo $totalRow?></span>
								 <span class="pagination__row">
                                                    ...
                                                    ?>
                                                    <a href="<?php echo base_url('News/Page/').$page2?> " class="pagination__cell pagination__cell--nav no-decoration">&laquo;</a>
                                    <?php for($i=1;$i<=$totalRow;$i++){ ?>                
                                                    <a href="<?php echo base_url('News/Page/').$i?> " class="pagination__cell no-decoration <?php if($page == $i){ echo 'pagination__cell--active'; }?>"><?php echo $i?></a>
                                    <?php }?>                 
                                                    <a href="<?php echo base_url('News/Page/').$page3?> " class="pagination__cell pagination__cell--nav no-decoration">&raquo;</a>
								</span>
							</div>
						</div>
